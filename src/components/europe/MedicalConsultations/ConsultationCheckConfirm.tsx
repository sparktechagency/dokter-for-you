        ...
        <div>
            <div className="">
                ...
                <div className={`bg-[#F3F6FF]  shadow-sm mb-4 ${selectedMedicines?.length === 0 ? 'hidden' : ''}`}>
                    {
                        selectedMedicines?.map((medicine: { name: string, medicineType: string, dosage: string, image: string, form: string, count: string, total: string, price: number }, index: number) => (
