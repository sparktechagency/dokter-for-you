...
const Search = () => { 
  ...
  return (
    <div className='bg-[#F7F7F7] '>
      ...
      {
        AllCategory?.categories?.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 py-[52px] container" >
          {AllCategory?.categories?.map((category: { _id: string, name: string, image: string, summary: string }, index: number) => (
